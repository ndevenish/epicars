...
It is here rather than in Phase 3 because Phase 2 makes the bug live.

> **Correction, from implementing 0.4:** this item is *not* the only Phase 0 change to
> the `Provider` signature, as this section originally claimed. Making the fan-out
> payload neutral necessarily moves `monitor_value`'s return type to
> `broadcast::Receiver<(Value, Meta)>`, since the trait is what hands that channel to
> the server. The alternatives — a second `Dbr` channel alongside the neutral one, or a
> forwarding task per PV — are both worse. Two Phase 0 items touch the trait, not one.
