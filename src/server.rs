...
impl<L: Provider> Server<L> {
    ...
    /// Start listening for searches, and return the bound port.
    ///
    ...
    /// with a search port of `0`, this will always either fail or
    /// return the same search port. If a search port of `0` _was_
    /// configured, then a random port will be bound and returned by
    /// this Result.
